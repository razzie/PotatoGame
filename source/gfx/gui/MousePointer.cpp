/*
 * Copyright (C) Gábor Görzsöny <gabor@gorzsony.com> - All Rights Reserved
 * Unauthorized copying of this file, via any medium is strictly prohibited
 * Proprietary and confidential
 */

#include <cmath>
#include "common/PI.hpp"
#include "gfx/gui/GUI.hpp"
#include "gfx/gui/MousePointer.hpp"

static unsigned char mouse[] = {
	 
};

static const float vertices[] = {
	0.f, 0.f,
	0.f, 1.f,
	1.f, 0.f,

	1.f, 1.f,
	1.f, 0.f,
	0.f, 1.f
};

static const char* vert = GLSL(
uniform mat4 transform;
in vec2 position;
out vec2 frag_position;
void main()
{
	frag_position = position;
	gl_Position = transform * vec4(position, 0.0, 1.0);
}
);

static const char* frag = GLSL(
uniform sampler2D texture;
in vec2 frag_position;
out vec4 out_color;
void main()
{
	out_color = texture2D(texture, frag_position);
}
);


gfx::gui::MousePointer::MousePointer(GUI& gui, GL::Window& window) :
	m_gl(gui.getContext()),
	m_shader(GL::Shader(GL::ShaderType::Vertex, vert), GL::Shader(GL::ShaderType::Fragment, frag)),
	m_vbo(vertices, sizeof(vertices), GL::BufferUsage::StaticCopy),
	m_proj(GL::Mat4::Ortho(0.f, (float)window.GetWidth(), (float)window.GetHeight(), 0.f, 0.f, 1000.f)),
	m_size(0.f)
{
	m_vao.BindAttribute(m_shader.GetAttribute("position"), m_vbo, GL::Type::Float, 2, sizeof(float) * 2, 0);

	resource::ArchiveReader& archive = gui.getArchive();
	resource::ArchiveReader::FileInfo file_info;

	if (archive.getFileInfo("mouse.png", file_info))
	{
		std::vector<unsigned char, raz::Allocator<unsigned char>> data;
		data.resize(file_info.size);
		archive.decompress(file_info.file_index, data.data());

		GL::Image image(data.data(), data.size());
		m_texture = image;
		m_size = (float)image.GetWidth();
	}
	else
	{
		GL::Image image(mouse, sizeof(mouse));
		m_texture = image;
		m_size = (float)image.GetWidth();
	}
}

void gfx::gui::MousePointer::update(const common::InputHelper& input)
{
	GL::Vec2 mouse_pos((float)input.getMousePosition().x, (float)input.getMousePosition().y);

	float angle = (float)std::atan2(mouse_pos.X - m_mouse_back_pos.X, m_mouse_back_pos.Y - mouse_pos.Y);
	m_mouse_back_pos.X = mouse_pos.X - std::sin(angle) * m_size;
	m_mouse_back_pos.Y = mouse_pos.Y + std::cos(angle) * m_size;

	GL::Mat4 transform;
	transform.Translate(GL::Vec3(mouse_pos.X, mouse_pos.Y, 0.f));
	transform.RotateZ(angle + 0.25f * (float)common::PI);
	transform.Scale(GL::Vec3(m_size, m_size, 0.f));

	m_transform = transform;
}

void gfx::gui::MousePointer::render()
{
	//m_gl.DepthMask(false);
	//m_gl.Disable(GL::Capability::DepthTest);

	//glEnable(GL_BLEND);
	//glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);

	//m_gl.Disable(GL::Capability::CullFace);

	m_gl.UseProgram(m_shader);

	m_gl.BindTexture(m_texture, 0);

	m_shader.SetUniform("transform", m_proj * m_transform);
	m_shader.SetUniform("texture", 0);

	m_gl.DrawArrays(m_vao, GL::Primitive::Triangles, 0, 6);
}
