...
static void insertVoronoiCell(const boost::polygon::voronoi_cell<double>& cell, raz::Random& random, float radius, ui::core::MeshBuffer<>& meshbuffer)
{
	...
	do
	{
		if (edge->is_primary())
		{
			...
			if (distance < radius)
			{
				ui::core::Vertex v{ GL::Vec3(point.x, height + 0.125f * (radius - distance), point.y), GL::Vec3(0.f, 1.f, 0.f), GL::Color(color, color, color) };
...
static void insertWires(const boost::polygon::voronoi_diagram<double>& diagram, raz::Random& random, float radius, unsigned complexity, ui::core::MeshBuffer<>& meshbuffer)
{
	...
	for (size_t i = 0; i < wires; ++i)
	{
		...
		ui::shape::WireShape wire(p1, p2, 8, drop);
		wire.generate(meshbuffer);
	}
}

ui::shape::VoronoiPillarShape::VoronoiPillarShape(float radius, unsigned complexity) :
	m_radius(radius),
	m_complexity(complexity)
{
}

void ui::shape::VoronoiPillarShape::generate(raz::Random& random, ui::core::MeshBuffer<>& meshbuffer) const
{
